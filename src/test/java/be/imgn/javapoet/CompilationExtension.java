...
public class CompilationExtension implements BeforeAllCallback, BeforeEachCallback,
  ...
  public void beforeAll(ExtensionContext context) throws InterruptedException {
    var state = context.getStore(NAMESPACE).getOrComputeIfAbsent(
      CompilerState.class,
      ignored -> new CompilerState(this.compilerExecutor, TestInstance.Lifecycle.PER_CLASS),
      CompilerState.class
    );

    if (!state.prepareForTests()) {
  ...
  public void afterEach(ExtensionContext context) throws Exception {
    var state = requireNonNull(context.getStore(NAMESPACE).get(
      CompilerState.class,
      CompilerState.class
    ));
    ...
    if (state.getLifecycle() == TestInstance.Lifecycle.PER_METHOD) {
      ...
      var compilation = state.allowTermination();
      if (!compilation.status().equals(SUCCESS)) {
        throw new IllegalStateException(String.valueOf(compilation));
      }
    }
  }
  ...
  public void afterAll(ExtensionContext context) throws ExecutionException, InterruptedException {
    ...
    if (state.getLifecycle() != TestInstance.Lifecycle.PER_CLASS) {
  ...
  public Object resolveParameter(ParameterContext parameterContext, ExtensionContext extensionContext)
    throws ParameterResolutionException {
    var state = extensionContext.getStore(NAMESPACE).get(
      CompilerState.class,
      CompilerState.class
    );

    if (state == null) {
      throw new IllegalStateException("CompilerState not initialized");
    }
  ...
  static final class CompilerState implements AutoCloseable {
    ...
    Compilation allowTermination() throws InterruptedException, ExecutionException {
      if (this.syncBarrier.getPhase() == 1) {
        if (this.syncBarrier.arriveAndDeregister() != 1) {
      ...
      try {
        var result = this.result.get(1, TimeUnit.SECONDS);
  ...
  static final class EvaluatingProcessor extends AbstractProcessor
    ...
    public synchronized void init(ProcessingEnvironment processingEnvironment) {
      ...
      if (!sharedState.compareAndSet(null, processingEnvironment)) {
        throw new IllegalStateException("Shared ProcessingEnvironment was already initialized");
      }
    }
    ...
    public Compilation get() {
      try {
        return javac().withProcessors(this).compile(DUMMY);
      } finally {
